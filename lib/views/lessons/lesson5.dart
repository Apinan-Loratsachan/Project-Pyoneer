  ...
  Widget build(BuildContext context) {
    return LessonScreenModel(
      ...
      contentWidgets: [
        ...
        PyoneerText.brakeLine(10),
        PyoneerText.contentText("คำสั่งรับข้อมูล",fontWeight: FontWeight.bold, fontSize: 20,),
        PyoneerText.contentText("ใช้ฟังก์ชั่น input() จะรอรับข้อมูลจากผู้ใช้ ดังนั้นเราต้องพิมพ์ข้อมูลเข้าผ่านทางคีย์บอร์ด ปกติเราใส่ข้อมูล 1 ตัว (เช่น ตัวเลข หรือ ข้อความ) แล้วเคาะ enter ข้อมูลที่เราพิมพ์นั้นจะเป็น string (function return string) เราต้องสร้างตัวแปร เพื่อเก็บข้อมูลดังกล่าว",tabSpace: true),
        PyoneerText.brakeLine(10),
        Row(
          children: [
            PyoneerText.contentText("",tabSpace: true),
        ...
        PyoneerText.contentText("test = input(\"Enter your input: \")",tabSpace: true),
        PyoneerText.contentText("print (\"Received input is : \", test)",tabSpace: true),
        PyoneerText.brakeLine(5),
        PyoneerText.contentText("หากผู้ใช้พิมพ์ข้อความ Hello Python ตัวอย่างการทำงานและผลลัพธ์ จะได้ดังนี้",tabSpace: true),
        PyoneerText.brakeLine(5),
        PyoneerText.contentText("Enter your input : Hello Python",tabSpace: true),
        PyoneerText.contentText("Received input is : Hello Python",tabSpace: true),
        PyoneerText.brakeLine(10),
        PyoneerText.contentText("จากตัวอย่าง ข้อความ Hello Python ที่เราพิมพ์ จะถูกเก็บไว้ที่ตัวแปร test ทำให้เห็นว่า ฟังก์ชั่น input จะรอผู้ใช้พิมพ์ข้อมูลจากคีย์บอร์ด ซึ่งข้อมูลที่รับมานั้นจะเป็น สตริง นำไปคำนวณไม่ได้ถึงแม้เราจะพิมพ์ตัวเลขก็ตาม หากเราต้องการนำไปคำนวณ เราต้องแปลงสตริงเป็นตัวเลขที่ต้องการ เช่น แปลงจาก string เป็น int หรือ แปลงจาก string เป็น float",tabSpace: true),
        PyoneerText.brakeLine(10),
        LessonComponent.lessonImage(
          context, 
        ...
        PyoneerText.contentText("บรรทัดที่ 1 จะปรากฏข้อความ Input integer number ที่จอภาพ เคอร์เซอร์กระพริบ รอให้ผู้ใช้พิมพ์ สมมติว่าผู้ใช้	พิมพ์ 55 เคาะ enter ผลที่ได้ คือ string 55 จะถูกเก็บที่ตัวแปร inp1",tabSpace: true),
        ...
        PyoneerText.brakeLine(10),
        PyoneerText.contentText("เมื่อโปรแกรมทำงาน หากเราใส่ 1.2 เก็บที่ตัวแปร โปรแกรมจะทำการเก็บข้อมูลที่เราป้อนเป็นแบบ string ในขั้นตอนนี้จะยังไม่มีปัญหาอะไร แต่ปัญหาจะเกิดขึ้นตอนที่เราเรียกใช้งาน ซึ่งในที่นี้เราเรียกใช้งานแบบ int จึงทำให้เกิด error",tabSpace: true),
